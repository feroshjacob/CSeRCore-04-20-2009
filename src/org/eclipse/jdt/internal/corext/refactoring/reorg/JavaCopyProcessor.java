...
import org.eclipse.jdt.internal.corext.refactoring.RenamedFile;
...
public final class JavaCopyProcessor extends CopyProcessor implements IReorgDestinationValidator {
	...
	public RefactoringStatus checkInitialConditions(IProgressMonitor pm) throws CoreException {
		...
		if(javaResources.length>0 && javaResources[0] instanceof IFile){
			//System.out.println("has a file");
			RenamedFile.setFile((IFile)javaResources[0]);
		}
	...
	public Change createChange(IProgressMonitor pm) throws CoreException {
		...
		Assert.isTrue(fCopyPolicy.getJavaElementDestination() != null || fCopyPolicy.getResourceDestination() != null);
		try {
			RenamedFile.setNewFilepath(fCopyPolicy.getJavaElementDestination().getCorrespondingResource().getLocation() +"/"+ RenamedFile.getFile().getName());
